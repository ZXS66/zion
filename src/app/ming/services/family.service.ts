import { Injectable } from "@angular/core";
import { HttpClient } from "@angular/common/http";

import { FamilyMember, NameValueChildren, NameValueTag } from "src/app/ming/ming.models";
import { API_BASE_URL, API_TOKEN } from "src/app/constants";
import { getCache, setCache, cache, removeCache, clearCache } from "src/app/cacheHelper"

@Injectable({ providedIn: "root" })
export class FamilyService {
	private baseUrl = API_BASE_URL + "family";
	constructor(private http: HttpClient) { }
	private async _get<T>(url: string): Promise<T> {
		let result = await getCache<T>(url);
		if (!result) {
			result = await this.http.get<T>(url, {
				headers: { "Content-Type": "application/json", "x-token": API_TOKEN },
				responseType: "json",
			}).toPromise();
			setCache(result, url);
		}
		return result;
	}
	private async _post<T>(url: string, body: Object = null): Promise<T> {
		// removeCache(url);
		clearCache();
		return await this.http.post<T>(
			url,
			body == null ? null : JSON.stringify(body),
			{
				headers: { "Content-Type": "application/json", "x-token": API_TOKEN },
				responseType: "json",
			}
		).toPromise();
	}

	searchFamilyMembers(
    /**search query*/ q: string,
    /** zero-based page index */ page_index = 0,
    /** how many items per page*/ page_size = 10,
	): Promise<FamilyMember[]> {
		const params = new URLSearchParams();
		params.set("q", q);
		params.set("page_index", page_index.toString());
		params.set("page_size", page_size.toString());
		const url = this.baseUrl + "/members/?" + params.toString();
		return this._get<FamilyMember[]>(url);
	}
	/** add a new family member */
	addFamilyMember(member: Partial<FamilyMember>): Promise<FamilyMember> {
		return this._post<FamilyMember>(this.baseUrl + "/members/", member);
	}
	/** get details of the family member by id */
	getFamilyMember(
    /**id of the family member*/ id: number,
	): Promise<FamilyMember> {
		return this._get<FamilyMember>(this.baseUrl + "/members/" + id);
	}
	/** update a family member */
	updateFamilyMember(
		id: number,
		member: Partial<FamilyMember>,
	): Promise<FamilyMember> {
		return this._post(this.baseUrl + "/members/" + id, member);
	}
	/** query metadata of id-name pairs */
	queryNameMeta(): Promise<NameValueTag[]> {
		return this._get<NameValueTag[]>(this.baseUrl + "/namemeta/");
	}
	queryHierarchy(): Promise<NameValueChildren> {
		return this._get<NameValueChildren>(this.baseUrl + "/hierarchy/");
	}
}
